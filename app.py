...
def load_data():
    if os.path.exists('data.csv'):
        ...
        st.error("data.csv not found. Please upload a file.")
        return pd.DataFrame()
...
if uploaded_file is not None:
    # Delete existing data.csv if it exists
    if os.path.exists('data.csv'):
        try:
            os.remove('data.csv')
            st.success("Existing data.csv file deleted successfully.")
        except Exception as e:
            st.error(f"Error deleting existing data.csv: {e}")

    # Read and encode the file
    file_content = uploaded_file.read()
    encoded_content = base64.b64encode(file_content).decode('utf-8')

    # Save locally as data.csv
    with open('data.csv', 'wb') as f:
        f.write(file_content)
        st.success("File saved locally as data.csv")
